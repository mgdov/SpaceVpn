    ...
    try {
      ...
      if (response.data?.bot_username) {
        ...
        const redirectUrl = encodeURIComponent(
          typeof window !== "undefined" ? window.location.origin + "/auth/telegram/callback" : ""
        )
        const telegramUrl = `https://oauth.telegram.org/auth?bot_id=${botUsername}&origin=${typeof window !== "undefined" ? window.location.origin : ""}&request_access=write&return_to=${redirectUrl}`
        if (typeof window !== "undefined") {
          sessionStorage.setItem(OAUTH_REDIRECT_KEY, redirectTo)
        }
        const w = 550
        ...
        window.open(telegramUrl, "telegram_auth", `width=${w},height=${h},left=${left},top=${top}`)
      } else {
  ...
  return (
    <div className="min-h-screen bg-background relative flex items-center justify-center px-3 sm:px-4 py-6 sm:py-8">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        {/* Social Register: Google, Telegram, Apple (скоро) */}
        <div className="flex flex-wrap gap-2 sm:gap-3 mb-4 sm:mb-6">
          {oauthProviders.some((p) => p.id === "google" && p.enabled) && (
            <button
              type="button"
          ...
          {oauthProviders.some((p) => p.id === "telegram" && p.enabled) && (
            <button
              type="button"
